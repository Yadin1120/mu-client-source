...
void PartObjectColor(int Type, float Alpha, float Bright, vec3_t Light, bool ExtraMon)
{
    ...
    else if (Type == MODEL_MUPASS_ROYAL)
    {
        Color = 46;
    }
    else if (Type == MODEL_MUPASS_FROST)
    {
        Color = 47;
    }
    else if (Type == MODEL_MUPASS_VOID)
    {
        Color = 48;
    }
    ...
    // MU Pass season 1 glow colours. The +11..+15 tiers stack three additive
    // bright passes, so a full-strength colour sums past 1.0 in every channel
    // and the glow washes out white. These are pre-divided by ~3 so the three
    // passes add up to the intended hue instead of saturating.
    // MU Pass season 1. FOUR additive bright layers stack on these swords: the
    // three glow passes of the +11..+15 tiers plus the blade sheen in
    // RenderPartObjectBody. Full-strength colours therefore sum past 1.0 in
    // every channel and wash out white, so each layer carries a quarter of the
    // intended colour and the four of them add up to it.
    case 46:Vector(Bright * 0.15f, Bright * 0.11f, Bright * 0.02f, Light); break; // royal gold
    case 47:Vector(Bright * 0.04f, Bright * 0.11f, Bright * 0.15f, Light); break; // frost cyan
    case 48:Vector(Bright * 0.10f, Bright * 0.04f, Bright * 0.15f, Light); break; // void violet
    }
}
...
void PartObjectColor2(int Type, float Alpha, float Bright, vec3_t Light, bool ExtraMon)
{
    ...
    // MU Pass season 1. This table modulates the incoming light rather than
    // replacing it, so the tints are written as multipliers. Kept above case 3,
    // which deliberately has no break.
    // Quarter-strength, matching PartObjectColor above.
    case 10: Vector(Bright * 0.15f * Light[0], Bright * 0.11f * Light[1], Bright * 0.02f * Light[2], Light); break; // royal gold
    case 11: Vector(Bright * 0.04f * Light[0], Bright * 0.11f * Light[1], Bright * 0.15f * Light[2], Light); break; // frost cyan
    case 12: Vector(Bright * 0.10f * Light[0], Bright * 0.04f * Light[1], Bright * 0.15f * Light[2], Light); break; // void violet
...
void PartObjectColor3(int Type, float Alpha, float Bright, vec3_t Light, bool ExtraMon)
{
    ...
    // MU Pass season 1. PartObjectColor3 is what RENDER_CHROME3 uses, i.e. the
    // pulsing Ancient halo, so the skins need their tint here too. Without this
    // the default Color of 0 would paint all three of them blue.
    if (Type == MODEL_MUPASS_ROYAL)
    {
        Color = 10;
    }
    else if (Type == MODEL_MUPASS_FROST)
    ...
    case 10: Vector(1.00f, 0.82f, 0.30f, Light); break; // royal gold
    case 11: Vector(0.35f, 0.80f, 1.00f, Light); break; // frost cyan
    case 12: Vector(0.72f, 0.32f, 1.00f, Light); break; // void violet
    }
}
...
void RenderPartObjectBody(BMD* b, OBJECT* o, int Type, float Alpha, int RenderType)
{
    ...
    else if (Type == MODEL_LORD_SCEPTER)
    {
        ...
        || (Type >= MODEL_MUPASS_ROYAL && Type <= MODEL_MUPASS_VOID))
    {
        b->RenderBody(RENDER_TEXTURE, Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, o->HiddenMesh);
        ...
        b->RenderMesh(1, RENDER_TEXTURE, Alpha, 1, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV);
        //. 날
        // The sheen laid over the blade. The stock Knight Blade is violet; each
        // MU Pass skin needs its own or the violet would fight the recolour.
        switch (Type)
        {
        // Quarter strength: this sheen is the fourth additive layer on top of
        // the three glow passes, so it carries the same share as each of them.
        case MODEL_MUPASS_ROYAL: glColor3f(0.15f, 0.11f, 0.02f); break;
        case MODEL_MUPASS_FROST: glColor3f(0.04f, 0.11f, 0.15f); break;
        case MODEL_MUPASS_VOID: glColor3f(0.10f, 0.04f, 0.15f); break;
        default: glColor3f(0.43f, 0.14f, 0.6f); break;
        }
...
void RenderPartObjectBodyColor(BMD* b, OBJECT* o, int Type, float Alpha, int RenderType, float Bright, int Texture, int iMonsterIndex)
{
    ...
    if (Type == MODEL_LEGENDARY_STAFF)
        ...
        || (Type >= MODEL_MUPASS_ROYAL && Type <= MODEL_MUPASS_VOID)) // MU Pass skins share the Knight Blade mesh layout
        b->RenderBody(RenderType, Alpha, o->BlendMesh, o->BlendMeshLight, o->BlendMeshTexCoordU, o->BlendMeshTexCoordV, 1, Texture);
    else if (Type == MODEL_DARK_REIGN_BLADE)
...
void RenderPartObjectEffect(OBJECT* o, int Type, vec3_t Light, float Alpha, int ItemLevel, int ExcellentFlags, int ancientDiscriminator, int Select, int RenderType)
{
    ...
    switch (Type)
    {
    // MU Pass season 1 cosmetics: glow at a fixed tier no matter what the item
    // level actually is, the same trick the transformation rings below use.
    case MODEL_MUPASS_ROYAL:
    case MODEL_MUPASS_FROST:
    case MODEL_MUPASS_VOID:Level = 13; break;
    ...
    // This cap is what lets a forced glow tier degrade gracefully: the ladder
    // below gates every tier on the render level too, so an uncapped Level of 13
    // would match no branch at all on render levels 1-3 and the item would come
    // out with no glow whatsoever. Leave the MU Pass skins capped like everything
    // else - they still always glow, just at the tier the player's setting allows.
    ...
    if (!o->EnableShadow)
    {
            ...
            )
        {
                ...
                && (o->Type != MODEL_WING + 135))
            {
                ...
                || (Type >= MODEL_MUPASS_ROYAL && Type <= MODEL_MUPASS_VOID))
            {
                // The MU Pass skins wear the pulsing halo permanently. Driving it
                // from the model instead of the ancient flag keeps it purely
                // cosmetic - no item set, no set bonus, nothing to grant.
