...
void OpenItems()
{
    ...
    // MU Pass season 1 - three cosmetic Knight Blades. They reuse Sword21.bmd
    // verbatim; only OpenItemTextures() points each one at its own texture folder.
    gLoadData.AccessModel(MODEL_MUPASS_ROYAL, L"Data\\Item\\", L"Sword", 21);
    gLoadData.AccessModel(MODEL_MUPASS_FROST, L"Data\\Item\\", L"Sword", 21);
    gLoadData.AccessModel(MODEL_MUPASS_VOID, L"Data\\Item\\", L"Sword", 21);
...
void OpenItemTextures()
{
    ...
    // MU Pass season 1 - same mesh, recoloured texture sets in their own folders.
    gLoadData.OpenTexture(MODEL_MUPASS_ROYAL, L"Item\\MuPassRoyal\\");
    gLoadData.OpenTexture(MODEL_MUPASS_FROST, L"Item\\MuPassFrost\\");
    gLoadData.OpenTexture(MODEL_MUPASS_VOID, L"Item\\MuPassVoid\\");
...
void OpenBasicData(HDC hDC)
{
    ...
    */
    ...
    // הטולטיפים של עץ המאסטר נטענו קשיח מתיקיית Eng, ולכן הם הוצגו
    // באנגלית גם בקליינט העברי — וגם התרגום של הקובץ המקביל בתיקיית he
    // לא היה עושה דבר. עכשיו הנתיב נבנה לפי השפה הנבחרת, כמו כל שאר
    // קובצי Data\Local.
    mu_swprintf(Text, L"Data\\Local\\%ls\\MasterSkillTooltip_%ls.bmd",
                g_strSelectedML.c_str(), g_strSelectedML.c_str());
    g_pMasterLevelInterface->OpenMasterSkillTooltip(Text);
