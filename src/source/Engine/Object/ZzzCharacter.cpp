...
// כמה מהירות ההתקפה והקסם מתורגמות למהירות ניגון האנימציה.
//
// היחס ישר. במקום זה ישבה כאן טבלת מקדמים יורדת שנועדה להקהות מהירויות גבוהות,
// אבל היו בה חורים ומדרגות כלפי מטה: 509-750 בהתקפה, ו-1041 ו-1301-1485 בהטלת
// לחשים. בטווחים האלה תוספת מהירות דווקא האטה את האנימציה - וכולם נמצאים בתוך
// הטווח שכל הדמויות חיות בו מאז בונוס המהירות הגלובלי. לקו ישר אין בורות כאלה,
// ולכן גם כיול מחדש של הבונוס בטוח בכל ערך.
constexpr float ATTACK_SPEED_TO_PLAY_SPEED = 0.004f;
constexpr float CAST_SPEED_TO_PLAY_SPEED = 0.002f;

void SetAttackSpeed()
{
    const float AttackSpeed1 = CharacterAttribute->AttackSpeed * ATTACK_SPEED_TO_PLAY_SPEED;
    const float MagicSpeed1 = CharacterAttribute->MagicSpeed * ATTACK_SPEED_TO_PLAY_SPEED;
    const float MagicSpeed2 = CharacterAttribute->MagicSpeed * CAST_SPEED_TO_PLAY_SPEED;
...
namespace
{
    // כמה AttackTime מתקדם בפעימה אחת.
    //
    // AttackTime הוא מונה קבוע: הוא מתקדם ב-1 לפעימה, וכל אפקטי הכישופים - הקליע שיוצא
    // מהיד, הפגיעה במטרה והצליל - נוצרים רק כשהוא מגיע ל-g_iLimitAttackTime (15), כלומר
    // אחרי 0.6 שניות. כל מכה חדשה מאפסת אותו ל-1. זה עבד כשמכה נמשכה כשנייה, אבל עם
    // בונוס המהירות הגלובלי מכה נמשכת כעשירית שנייה - המונה מתאפס לפני שהספיק להגיע ל-15,
    // והאפקטים פשוט לא נוצרים אף פעם. אצל אביר זה לא בלט כי רואים את נפנוף החרב עצמו,
    // אבל לקוסם אין נפנוף וההתקפה שלו נראתה בלתי נראית לגמרי.
    //
    // הפתרון: להאיץ את המונה באותו יחס שבו הואצה האנימציה, כך שהוא מגיע ל-15 באותה נקודה
    // יחסית בתוך הנפנוף כמו קודם. מתחת למהירות הייחוס ההתנהגות זהה לחלוטין למקור.
    //
    // מחיר ידוע: בקפיצות גדולות המונה מדלג על ערכים שלמים, ולכן אפקטי ביניים שנתלים על
    // ערך מדויק (CheckAttackTime(5) וכדומה) עלולים לא להתנגן. עדיף על המצב הקודם שבו
    // *שום* אפקט לא נוצר.
    constexpr float ATTACK_TIME_REFERENCE_PLAY_SPEED = 0.5f;

    float AttackTimeStep(const OBJECT* o)
    {
        const float step = FPS_ANIMATION_FACTOR;
        if (o == nullptr)
            return step;

        // כישופי המזמנת (שינה, ברק משורשר, מציצת חיים) יושבים מחוץ לטווח של
        // IsAttackAction בפער של פעולה אחת, אבל האנימציה שלהם מואצת בדיוק כמו כל
        // השאר. בלי השורה הזו האנימציה נגמרת ברבע שנייה בעוד המונה עדיין צריך חצי -
        // האפקט מופיע אחרי שהדמות כבר חזרה לעמוד, ואם מטילים שוב בינתיים הוא נבלע.
        if (!Engine::Object::IsAttackAction(o->CurrentAction)
            && !Engine::Object::IsSpeedScaledSkillAction(o->CurrentAction))
            return step;

        const BMD* b = &Models[o->Type];
        if (b->NumActions <= 0 || o->CurrentAction >= b->NumActions)
            return step;

        const float playSpeed = b->Actions[o->CurrentAction].PlaySpeed;
        if (playSpeed <= ATTACK_TIME_REFERENCE_PLAY_SPEED)
            return step;

        return step * (playSpeed / ATTACK_TIME_REFERENCE_PLAY_SPEED);
    }
}
...
namespace
{
    bool IsForcedNpcMonsterType(EMonsterType type)
    {
        ...
        case 376:
        case 377:
        case 378: // GameMaster NPC (MONSTER_GAMEMASTER) - must be classified as NPC so it renders and is clickable
        case 379:
        case 380:
        case 381:
        case 382:
