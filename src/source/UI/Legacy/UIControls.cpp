...
// --- Additive Hebrew RTL (bidi) helper: reverse Hebrew runs so logical-order
// --- Hebrew renders correctly through the left-to-right glyph pipeline. ---
static const wchar_t* ApplyHebrewBidi(const wchar_t* in, wchar_t* buf, size_t bufLen)
{
    if (in == nullptr) return in;
    size_t n = 0; bool hasHeb = false;
    for (const wchar_t* p = in; *p; ++p) { if (*p >= 0x0590 && *p <= 0x05FF) hasHeb = true; ++n; }
    if (!hasHeb || n + 1 > bufLen) return in;
    for (size_t i = 0; i < n; ++i) buf[i] = in[n - 1 - i];
    buf[n] = 0;
    // re-reverse maximal runs of ASCII letters/digits so numbers/latin read forward
    size_t i = 0;
    while (i < n) {
        wchar_t c = buf[i];
        ...
        if (alnum) {
            size_t j = i;
            while (j < n) { wchar_t d = buf[j];
                bool a2 = (d >= L'0' && d <= L'9') || (d >= L'A' && d <= L'Z') || (d >= L'a' && d <= L'z');
                if (!a2) break; ++j; }
            for (size_t a = i, b = j - 1; a < b; ++a, --b) { wchar_t t = buf[a]; buf[a] = buf[b]; buf[b] = t; }
            i = j;
        } else ++i;
    }
    return buf;
...
#ifdef _WIN32
// --- Hebrew (bidi) base direction. GDI runs the Unicode bidi algorithm itself, but always with a
// --- left-to-right base direction. In a Hebrew line which also carries English words, an item
// --- name or a player name, the Hebrew parts are then placed in the wrong order and the sentence
// --- reads backwards ("הפלתי Dragon Blade +15 לידך." came out starting from "לידך"). Asking GDI
// --- for a right-to-left base direction fixes exactly that, and also puts the final period on the
// --- left where it belongs. The direction is decided by the first letter of the line, which is
// --- the Unicode rule, so lines starting in English render exactly as they did before.
static bool StartsWithHebrew(const wchar_t* text)
{
    for (const wchar_t* p = text; *p; ++p)
    {
        if ((*p >= 0x0590 && *p <= 0x05FF) || (*p >= 0xFB1D && *p <= 0xFB4F))
            return true;   // Hebrew letters
        if ((*p >= L'A' && *p <= L'Z') || (*p >= L'a' && *p <= L'z'))
            return false;  // latin letters
        if (*p >= 0x00C0 && *p <= 0x058F)
            return false;  // accented latin, greek, cyrillic
    }

    return false;          // digits and punctuation alone: keep the previous direction
}
#endif

void DrawTextDirectional(HDC hdc, int iPos_x, int iPos_y, const wchar_t* pszText, int iLength)
{
#ifdef _WIN32
    if (StartsWithHebrew(pszText))
    {
                                       ...
                                       int iSort /* = RT3_SORT_LEFT */, OUT SIZE* lpTextSize /* = NULL */)
{
    ...
    if (pszText[0] != 0x0a)
    {
        ...
        DrawTextDirectional(m_hFontDC, 0, 0, pszText, lstrlen(pszText));
    }
