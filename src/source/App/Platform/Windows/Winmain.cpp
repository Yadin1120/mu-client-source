...
#endif
{
    ...
    // Runs the client-side security self check once, before anything else needs
    // its result. A few milliseconds on a cold path; see SelfCheck.h for what it
    // is worth and what it is not.
    GameLogic::AntiCheat::Run();
    ApplyLaunchTokenToSerial();
