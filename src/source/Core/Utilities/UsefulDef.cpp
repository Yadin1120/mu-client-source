...
int DivideStringByPixel(wchar_t* alpszDst, int nDstRow, int nDstColumn, const wchar_t* lpszSrc, int nPixelPerLine, bool bSpaceInsert, const wchar_t szNewlineChar)
{
    ...
    // CutText3 doesn't reliably null-terminate every produced line, so any line
    // rendered afterwards exposed uninitialized memory (a row of identical
    // garbage glyphs, 0xCDCD in debug builds) in the shop dialogs. Clearing the
    // whole destination up front makes every caller safe.
    memset(alpszDst, 0, sizeof(wchar_t) * nDstRow * nDstColumn);
