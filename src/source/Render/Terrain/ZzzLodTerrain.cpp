...
int OpenTerrainAttribute(wchar_t* FileName)
{
    ...
    // Old attribute files only used the low 7 bits, so the original client
    // treated any value >= 0x80 as a tampered file. TW_CAMERA_UP (0x80) is a
    // valid flag in Season 6 maps - Atlans, Icarus, Kalima, Castle Siege,
    // Valley of Loren and Crywolf all carry it in the unused rows at the end
    // of the file - so it must not be reported as a data error.
    // The mask keeps the legacy single-byte attribute range; extended (WORD)
    // attributes lose their high byte here, which is pre-existing behaviour.
    for (int i = 0; i < TERRAIN_SIZE * TERRAIN_SIZE; i++)
    {
