...
{
    // sizeof(char) here was a leftover from when these were char[]: BuffInfo declares them as
    // wchar_t[], so only the first half of each buffer was cleared and the rest stayed garbage.
    // IsEqualBuffType() does wcscpy() straight out of s_BuffName, so an entry that never got
    // loaded from the script had no terminator in the cleared half - the copy ran off the end
    // of the array. That is also where the stray glyphs in the shop dialogs came from.
