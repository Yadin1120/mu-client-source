...
bool Buff::IsEqualBuffType(IN int iBuffType, OUT wchar_t* szBuffName)
{
    ...
    while (iter != m_Buff.end())
    {
        ...
        {
            // Bounded copy: s_BuffName comes from a script file and a malformed or missing
            // entry can leave it without a terminator. wcscpy would then run past the array.
            wcsncpy(szBuffName, buffinfo.s_BuffName, MAX_BUFF_NAME_LENGTH - 1);
            szBuffName[MAX_BUFF_NAME_LENGTH - 1] = L'\0';
            return true;
        }
