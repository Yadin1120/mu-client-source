...
namespace giPetManager
{
    ...
    void SetPetItemConvert(ITEM* ip, PET_INFO* pPetInfo)
    {
        ...
        {
            // Index 0 is a valid position, so "not found" needs a -1 sentinel; the original
            // Index==0 check re-appended the option every call when it sat at slot 0, growing
            // SpecialNum without bound and writing past Special[MAX_ITEM_SPECIAL].
            int Index = -1;
            for (int i = 0; i < ip->SpecialNum && i < MAX_ITEM_SPECIAL; ++i)
            {
    ...
    bool RenderPetItemInfo(int sx, int sy, ITEM* pItem, int iInvenType)
    {
        ...
        // Snapshot the count and clamp to the array size: SetPetItemConvert may grow SpecialNum,
        // and iterating a live, growing count is what corrupted memory here before.
        const int specialCount = (pItem->SpecialNum < MAX_ITEM_SPECIAL) ? pItem->SpecialNum : MAX_ITEM_SPECIAL;
