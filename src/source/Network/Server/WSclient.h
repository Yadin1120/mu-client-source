...
// אותה רשימה בדיוק, אבל עם כותרת C2 (אורך דו-בייטי) במקום C1 (אורך של בייט אחד).
//
// C1 מגביל את הפקטה כולה ל-255 בייט. כל דמות תופסת 44 בייט, ולכן שש דמויות (272 בייט)
// כבר לא נכנסות והשרת משדר פקטה קטועה — חשבון כזה פשוט לא הצליח להיכנס למשחק
// (אובחן 03/08/2026). עם C2 התקרה היא 65,535.
//
// ההבדל היחיד הוא **בייט אחד** בכותרת: C1 הוא קוד+אורך+סוג (3 בייט) ו-C2 הוא
// קוד+אורך-גבוה+אורך-נמוך+סוג (4 בייט). לכן כל השדות זזים במקום אחד, וגם כל רשומת
// דמות אחריהם. השרת שולח את הגרסה הזאת רק לקליינטים שמזדהים כעונה 107 ומעלה.
typedef struct
{
    PWMSG_HEADER Header;
    BYTE         SubCode;
    ...
    BYTE         CharacterCount;
    BYTE		 IsVaultExtended;
} PHEADER_DEFAULT_CHARACTER_LIST_C2, * LPPHEADER_DEFAULT_CHARACTER_LIST_C2;
#pragma pack(pop)

// מחוללי הגדלים האוטומטיים (wire_sizes.generated.h) מכסים רק פקטות באורך קבוע,
// ורשימת הדמויות היא באורך משתנה — ולכן היא לא מוגנת שם. הבדיקות כאן ידניות בכוונה:
// טעות של בייט אחד בפריסת הכותרות היא בדיוק מה שמפיל את מסך בחירת הדמויות, והיא
// חייבת להיתפס בזמן בנייה ולא אצל שחקן.
static_assert(sizeof(PHEADER_DEFAULT_CHARACTER_LIST) == 8,
              "C1 character-list header must stay 8 bytes (3 header + subcode + 4 fields)");
static_assert(sizeof(PHEADER_DEFAULT_CHARACTER_LIST_C2) == 9,
              "C2 character-list header must stay 9 bytes (4 header + subcode + 4 fields)");
static_assert(sizeof(PHEADER_DEFAULT_CHARACTER_LIST_C2) == sizeof(PHEADER_DEFAULT_CHARACTER_LIST) + 1,
              "the C2 header differs from C1 by exactly one length byte");
...
// Copies the launcher's one-time secret into the protocol serial field, so the
// server can tell a game that was opened through the launcher from one that was
// started directly. Does nothing when no secret was passed.
...
typedef struct 
{
    ...
    BYTE bStayInPlace : 1;                   // Index: 33 (bit 4)
