...
// תעודת הזהות שהקליינט מכריז עליה בהתחברות. הועלתה מ-20404 ל-20405 (עונה 106 → 107)
// כדי שהשרת יוכל **להבחין** בין קליינט ישן לחדש ולשלוח לכל אחד את הפורמט שהוא מבין:
...
{
    // הלאנצ'ר מגריל סוד חד-פעמי, רושם אותו בשרת ומעביר אותו כאן ב-/t. הסוד
    // נכנס בדיוק לשדה המספר הסידורי שכבר קיים בפרוטוקול (16 תווים), ולכן אין
    // צורך בפקטה חדשה ובאף שינוי בשכבת הרשת. השרת מזהה כך משחק שנפתח ישירות
    // בלי הלאנצ'ר, כלומר עקף את בדיקת הקבצים.
    const std::wstring& strToken = GameLogic::AntiCheat::GetReport().strLaunchToken;
    if (strToken.size() != SIZE_PROTOCOLSERIAL)
    {
        return;
    }

    for (size_t i = 0; i < SIZE_PROTOCOLSERIAL; i++)
    {
        Serial[i] = static_cast<BYTE>(strToken[i]);
    }

    Serial[SIZE_PROTOCOLSERIAL] = 0;
}
...
// ── מיקומי העמידה של הדמויות במסך בחירת הדמויות (World74) ─────────────────
//
// המצלמה של מסך הבחירה קבועה בנקודה אחת (WALK_PATHS[5] ב-LoginScene.cpp),
// ולכן כל משבצת חייבת ליפול בתוך שדה הראייה שלה. שבע המשבצות פרושות על
// **אותה קשת** שעליה עמדו חמש הדמויות המקוריות — מהנקודה הראשונה ועד
// האחרונה, במרווחים אחידים לפי אורך הקשת. כך השטח, המרחק מהמצלמה והמסגור
// נשארים בדיוק כשהיו, רק עם שבע דמויות במקום חמש.
//
// הטבלה משותפת ל-ReceiveCharacterListExtended ול-ReceiveCreateCharacter:
// קודם היו כאן שתי רשימות מיקומים כפולות, ודמות שזה עתה נוצרה הייתה עלולה
// לעמוד במקום אחר מזה שבו תופיע בכניסה הבאה.
struct CHARACTER_SLOT_POSITION
{
    float fX;
    float fY;
    float fAngle;
...
static constexpr CHARACTER_SLOT_POSITION g_CharacterSlotPositions[MAX_CHARACTERS_PER_ACCOUNT] =
{
    { 8008.0f, 18885.0f, 115.0f },
    { 7993.0f, 19056.0f,  98.5f },
    { 8005.0f, 19226.0f,  85.0f },
    { 8044.0f, 19393.0f,  75.5f },
    { 8101.0f, 19556.0f,  65.5f },
    { 8179.0f, 19707.0f,  52.0f },
    { 8282.0f, 19845.0f,  35.0f },
};

// מחזירה false אם השרת שלח משבצת שהבנייה הזאת של הקליינט לא מכירה.
static bool GetCharacterSlotPosition(int iSlot, float* pfPos, float* pfAngle)
{
    if (iSlot < 0 || iSlot >= MAX_CHARACTERS_PER_ACCOUNT)
    {
        return false;
    }

    pfPos[0] = g_CharacterSlotPositions[iSlot].fX;
    pfPos[1] = g_CharacterSlotPositions[iSlot].fY;
    *pfAngle = g_CharacterSlotPositions[iSlot].fAngle;
    return true;
}
...
void ReceiveCharacterListExtended(const BYTE* ReceiveBuffer)
{
    ...
    // הבנייה הזאת מבינה את **שתי** צורות הכותרת של אותה הודעה:
    //   C1 — אורך של בייט אחד, ולכן תקרה של 5 דמויות (44 בייט לדמות + 8)
    //   C2 — אורך דו-בייטי, בלי התקרה הזאת
    // השרת בוחר לכל חיבור בנפרד לפי גרסת הקליינט שהוכרזה בהתחברות, ולכן אותו קובץ
    // עובד מול שרת ישן ומול שרת חדש. זה מה שמאפשר להפיץ את הקליינט לפני שמשנים את
    // השרת, בלי חלון שבו מישהו ננעל בחוץ.
    BYTE MaxClass;
    BYTE CharacterCount;
    BYTE IsVaultExtended;
    int  Offset;

    if (ReceiveBuffer[0] == 0xC2)
    {
        auto Data = (LPPHEADER_DEFAULT_CHARACTER_LIST_C2)ReceiveBuffer;
        MaxClass        = Data->MaxClass;
        CharacterCount  = Data->CharacterCount;
        IsVaultExtended = Data->IsVaultExtended;
        Offset          = sizeof(PHEADER_DEFAULT_CHARACTER_LIST_C2);
    }
    else
    {
        ...
        CharacterCount  = Data->CharacterCount;
        IsVaultExtended = Data->IsVaultExtended;
        Offset          = sizeof(PHEADER_DEFAULT_CHARACTER_LIST);
    }

#ifdef _DEBUG
    g_ConsoleDebug->Write(MCD_RECEIVE, L"[ReceiveList Count %d Max class %d Header %02X]", CharacterCount, MaxClass, ReceiveBuffer[0]);
#else
    g_ErrorReport.Write(L"[ReceiveList Count %d Max class %d Header %02X]", CharacterCount, MaxClass, ReceiveBuffer[0]);
#endif

    CharacterAttribute->IsVaultExtended = IsVaultExtended;
    for (int i = 0; i < CharacterCount; i++)
    {
        ...
        // משבצת שהקליינט לא מכיר — מדלגים עליה וממשיכים לשאר הרשימה.
        //
        // כאן היה `default: return`, שנטש את כל הפענוח **לפני** השורה שמסמנת
        // בסוף הפונקציה שהרשימה התקבלה. חשבון עם דמות במשבצת מעבר לתקרה של
        // הקליינט נתקע כך לפני הלובי בלי שום הודעת שגיאה, ולפעמים גם קרס
        // בציור סצנה חצי-בנויה (אובחן 03/08/2026 על חשבון עם 6 דמויות).
        // דילוג משאיר את הלובי פתוח עם מה שכן מוכר.
        if (!GetCharacterSlotPosition(Data2->Index, fPos, &fAngle))
        {
            continue;
        }
...
void ReceiveCreateCharacter(const BYTE* ReceiveBuffer)
{
    ...
    if (Data->Result == 1)
    {
        ...
        // אותה טבלה שמשמשת את טעינת רשימת הדמויות, כדי שדמות שזה עתה נוצרה
        // תעמוד בדיוק במקום שבו היא תופיע בכניסה הבאה.
        if (!GetCharacterSlotPosition(Data->Index, fPos, &fAngle))
        {
            return;
        }
...
static void ProcessPacket(const BYTE* ReceiveBuffer, int32_t Size)
{
    ...
    case 0xD2:
    {
        // The MU Pass list packets (0x21, 0x22) are C2 packets whose sub-code sits
        // after the 2-byte length - the C1 header cast below would misread them.
        if (ReceiveBuffer[0] == 0xC2)
        {
            switch (ReceiveBuffer[4])
            {
            case 0x21:
                Network::MuPass::ReceiveMissionList(ReceiveBuffer);
                break;
            case 0x22:
