...
namespace
{
    // X-shop catalog source: read from shop.ini next to the exe, so the server
    // address can change without recompiling (e.g. later switch to a LAN IP).
    // A missing file or key falls back to the local server defaults.
    struct XShopSource
    {
        wchar_t Host[128];
        unsigned short Port;
    ...
    // Bounded copy into a fixed-size wide buffer, always null-terminated. The shop.ini values are up
    // to 127 chars, so a plain wcscpy into a smaller destination overflows it - which is exactly what
    // happened with the host field once a real domain name (longer than an IPv4 literal) was used.
    template <size_t N>
...
bool CInGameShopSystem::BannerDownload()
{
    ...
    // Same source override as ScriptDownload: our server serves an (empty) banner
    // file, so the download succeeds silently and no error popup appears.
    const XShopSource source = ReadXShopSource();
    CopyBounded(m_szBannerIPAddress, source.Host);
    CopyBounded(m_szBannerRemotePath, source.BannerPath);
