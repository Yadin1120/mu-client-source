...
void CNewUIInGameShop::RenderTexts()
{
    ...
    // שורת "נקודות גובלין" הוסתרה: השרת שולח שם 0 קבוע (נקודות ה-MU Pass לא ניתנות להוצאה
    // בחנות ומוצגות בחלון של הפס), אז השורה רק הציגה 0 לנצח ונראתה כמו תקלה.
    if (g_InGameShopSystem->GetTotalMileage() > 0)
    {
...
bool CNewUIInGameShop::BtnProcess()
{
    ...
    if (m_UseButton.UpdateMouseEvent() == true)
    {
        ...
        // GetSelectedText() returns NULL whenever no line is selected - having lines is not the
        // same as having a selection, so the GetLineNum() check above does not cover this.
        if (pSelectItem == NULL)
        {
            CMsgBoxIGSCommon* pMsgBox = NULL;
            CreateMessageBox(MSGBOX_LAYOUT_CLASS(CMsgBoxIGSCommonLayout), &pMsgBox);
