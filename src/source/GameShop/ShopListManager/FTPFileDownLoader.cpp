    ...
    std::vector<std::wstring>	vScriptFiles) // OK
{
    ...
    // Respect the requested protocol. This was hardcoded to FTP, so the HTTP
    // catalog download silently tried an FTP handshake against the HTTP server
    // and died on the 3s connect timeout.
