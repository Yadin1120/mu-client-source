...
WZResult 			FileDownloader::TransferRemoteFile()
{
    ...
    if (this->CanBeContinue())
    {
        while (true)
        {
            ...
            // Read the next chunk. The decompiled loop never re-read inside the
            // loop, so a file that arrived in a single read was re-written
            // endlessly until the watchdog broke the download.
            ReadSize = 0;
            this->m_Result = this->m_pConnecter->ReadRemoteFile(this->m_hRemoteFile, buffer, &ReadSize);
