...
CALLBACK_RESULT CMsgBoxIGSStorageItemInfo::OKButtonDown(class CNewUIMessageBoxBase* pOwner, const leaf::xstreambuf& xParam)
{
    ...
    if (pMsgBox == NULL)
    {
        // Creating the confirm dialog can fail (layout/texture load); calling into it anyway
        // dereferences NULL and takes the whole client down on the "use" click.
        return CALLBACK_BREAK;
    }
...
#if defined(FOR_WORK) || defined(XSHOP_DEBUG_OVERLAY)
    // debug
